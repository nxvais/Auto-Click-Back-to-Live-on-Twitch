...
// @name         AdBlock Twitch
// @version      15.0.0
// @description  AD Block para Twitch que mantem a Qualidade
// @match        *://*.twitch.tv/*
// @author       9ais
...
(function() {
    ...
    createNotification("Script injetado com sucesso!", 0, 10);

    setTimeout(function() {
        createNotification("AdBlock 9ais Ativado!", 0, 10);
    }, 3000);


    var ourTwitchAdSolutionsVersion = 1;
            ...
            constructor(twitchBlobUrl, options) {
                ...
                this.addEventListener('message', (e) => {
                    if (e.data.key == 'ShowAdBlockBanner') {
                        ...
                        adBlockDiv.P.textContent = 'Tankando o AD FDSKKKKKKKKKK';
                        ...
                        //Isso é usado para corrigir o bug em que o vídeo congelava.
                        try {
                ...
                function getAdBlockDiv() {
                    //Para exibir uma notificação ao usuário de que um anúncio está sendo bloqueado.
                    ...
                    return new Promise(function(resolve, reject) {
                        var processAfter = async function(response) {
                            if (response.status === 200) {
                                //Aqui verificamos se há anúncios no menu e também tentamos tipos de players alternativos, se necessário.
    ...
    async function processM3U8(url, textStr, realFetch, playerType) {
        ...
        if (haveAdTags) {
            ...
            // Mantenha o m3u8 por um tempo (uma vez por anúncio) antes de solicitar um novo
            ...
            if (WasShowingAd) {
                ...
                WasShowingAd = false;
                //Aqui colocamos o player de volta à qualidade original e removemos a mensagem de bloqueio.
                postMessage({
                    key: 'ForceChangeQuality',
                    value: 'original'
                });
    ...
    function doTwitchPlayerTask(isPausePlay, isCheckQuality, isCorrectBuffer, isAutoQuality, setAutoQuality) {
        //Isso fará uma pausa/reprodução instantânea para retornar à qualidade original assim que o anúncio terminar.
        //Também usamos essa função para obter a qualidade atual do player de vídeo definida pelo usuário.
        //Também usamos essa função para pausar/reproduzir rapidamente o player ao alternar as guias para interromper atrasos.
        try {
            ...
            //Isso só acontece ao alternar entre abas e é para corrigir a alta latência causada ao abrir abas em segundo plano e ir para elas mais tarde.
            //Verificamos se esta é uma transmissão ao vivo pela URL da página, para evitar pausas/reproduções de vod/clipe.
            try {
                ...
                if (isCorrectBuffer && isLive) {
                   //Um cronômetro é necessário porque o jogador não pode continuar sem ele.
                    setTimeout(function() {
                        //Se a latência para o transmissor estiver acima de 5 ou 15 segundos ao alternar as guias, pausamos e reproduzimos o player para redefinir a latência.
                        //Se a latência estiver entre 0-6, o usuário pode pausar e retomar manualmente para redefinir a latência ainda mais.
        ...
        window.fetch = function(url, init, ...args) {
            if (typeof url === 'string') {
                ...
                if (url.includes('/access_token') || url.includes('gql')) {
                    //O ID do dispositivo é usado ao notificar o Twitch sobre anúncios.
                    var deviceId = init.headers['X-Device-Id'];
                    if (typeof deviceId !== 'string') {
                        deviceId = init.headers['Device-ID'];
                    }
                    //Adicionado para evitar eventuais conflitos de UBlock.
